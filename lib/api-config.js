...
export class APIKeyManager {
  ...
  async setupAPIKeyInteractively() {
    ...
    console.log(chalk.white('4. 💾 Copiez la clé (chaîne alphanumérique)'));
    ...
    const { apiKey } = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          ...
          // Together.ai utilise des clés alphanumériques, pas le format sk-
          if (input.length < 20) {
            return 'La clé API semble trop courte (minimum 20 caractères)';
          }
          // Vérifier que c'est bien une chaîne alphanumérique
          if (!/^[a-zA-Z0-9]+$/.test(input)) {
            return 'La clé API Together.ai doit contenir uniquement des lettres et des chiffres';
          }
          return true;
        }
      }
...
${this.apiKeyName}=votre-cle-api-together-ai
...
`;
  ...
  saveAPIKey(apiKey) {
    try {
      const envContent = `${this.apiKeyName}=${apiKey}\n`;
